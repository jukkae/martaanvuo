...
(define (display-pc-combatant-info actor)
  ...
  (when [pc-has-sense-organ? 'basic-homeostasis]
    ...
    (for ([percept percepts])
      (append-element! body (tr "basic homeostasis" "base feeling" percept))))

  (when (and (pc-has-sense-organ? 'eyes) (not (equal? (get-current-light-level) 'pitch-black)))
    (define percept
      (cond
        [(= (actor-hp actor) (actor-max-hp actor)) "Perfect"]
        [(> (actor-hp actor) (/ (actor-max-hp actor) 2)) "Somewhat damaged"]
        [else "Bad"]))
    (append-element! body (tr "vision" "organism condition" percept)))

  (when [pc-has-sense-organ? 'nociception]
    (append-element! body
                     (tr "nociception" "HP" (format "~a/~a" (actor-hp actor) (actor-max-hp actor)))))

  (when (not (null? (actor-statuses actor)))
    (define statuses (actor-statuses actor))
    (define statuses-strings
      (for/list ([status statuses])
        (format "[~a (~a)]" (status-type status) (status-lifetime status))))

    (define statuses-list (tr "" "statuses" (string-join statuses-strings)))
    ...
    (define conditions-strings
      (for/list ([condition conditions])
        (format "[~a~a (age: ~a ι)]"
                ...
                (if (Illness? condition) (format " – ~a" (Illness-state condition)) "")
                (condition-age condition))))

    (define conditions-list (tr "" "conditions" (string-join conditions-strings)))
    ...
    (case (stance-range (actor-stance enemy))
      ...
      ['nearby
       (p
        "The "
        (car enemy-names)
        " is a couple of paces away from Otava, trying to get closer. Otava is holding her revolver.")]
      #;
      ['close ; this is specific to enemy type etc
       (p
        "The "
        (car enemy-names)
        " is closing in fast, its claws flickering as it jumps from a rock onto the trunk of a nearby tree.")])))

(define (get-combatant-name actor)
  (cond
    [(pc-actor? actor) "Otava"]
    [else
     (when actor
       (define stance (actor-stance actor))
       (cond
         [(= (length (get-current-enemies)) 1) (actor-name actor)]
         [else
          (define name (actor-name actor))
          (define sign (if stance (stance-sign stance) ""))
          (cond
            [(equal? "" sign) name]
...
(define (display-non-pc-combatant-info actor)
       ...
       (case (get-current-light-level)
         ['bright
          ...
          (when (not (equal? (get-trait actor "visual description") 'not-found))
            (append-element!
             body
             (tr eyes-text "description: " (format "~a" (get-trait actor "visual description")))))]
         ['dark
          (append-element!
           body
           (cond
  ...
  (when (not (null? (actor-statuses actor)))
    (define statuses (actor-statuses actor))
    (define statuses-list (tr "statuses" (~s statuses) ""))
    (append-element! body (tr "" "" ""))
    (append-element! body statuses-list))
  (when (not (null? (actor-conditions actor)))
    (define conditions (actor-conditions actor))
    (define conditions-list (tr "conditions" (~s conditions) ""))
    (append-element! body (tr "" "" ""))
