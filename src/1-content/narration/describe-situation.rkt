...
(define (describe-location repeated?)
  ...
  (case (location-id (current-location))
    ['burnt-tree
     (p
      "The vengeful husk of the burnt tree, a vast deep black silhouette against the sky. Near ground, there's a slit in its outer shell, just wide enough for Otava to fit through. Inside, nothing but black and and an intense smell of ash.")]

    ['outpost
     (p
      "The building is a research outpost. Outside the main building, there's a big, drill-like machine puncturing the rock. Inside the building, there's more heavy machinery, a steel platform, a bench and some empty closets. Stairs lead down to a tunnel entrance.
  ...
  (define body
    (case (location-id (current-location))
      ['perimeter (Place-get-perceptions (current-location))]
      ['magpie-hill
       ...
       (when (pc-has-sense-organ? 'nose)
         (set! unpruned-rows
               (append-element unpruned-rows
                               (tr "Smells      [perceived with nose]"
                                   "Arid smell of desert flowers"))))
       (when (pc-has-sense-organ? 'ears)
         (set! unpruned-rows
               (append-element unpruned-rows
                               (tr "Sounds      [perceived with ears]"
