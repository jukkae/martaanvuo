...
(define (display-place-info-card location [title "Location"])
  ...
  (define body
    (prune (tbody (when (not (equal? (Place-shortname location) ""))
                  ...
                  (when (and (null? (location-id location)) (not (null? (location-type location))))
                    (tr "type" (symbol->string (location-type location))))
                  (when (not (null? (location-items location)))
                    (tr "items" (~v (location-items location))))
                  (when (not (null? (location-features location)))
                    (tr "features" (~v (location-features location)))))))
