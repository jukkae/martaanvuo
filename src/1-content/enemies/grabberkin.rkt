  ...
  (case action-flag
    ['pull-under
     ...
     (make-action
      ...
      #:resolution-rules
      `((p
         "The hands grasping her ankle – the thing with the hands – shift in the waters under the floating raft of moss. The thing pulls Otava through the moss, through a thick layer of algae, into the cloudy waters. The heavy, dark water closes in around her.")
        (p
         "The thing pulls her deeper. She fights back, but her arms get caught in the massive algae congesting the grimy waters. She cannot hold her breath much longer.") ; -> fragments -> saving throw, not direct death
        (wait-for-confirm)
        (p
         "Otava opens her mouth and drowns four feet under the surface of a nameless pool in Martaanvuo.")
    ...
    ['anklebreaker
     ...
     (make-action
      ...
      `((define target (pc))
        (cond
          [(not (actor-has-condition-of-type? target 'ankle-broken)) ; first
           (p
            "The hands tighten their vice-like hold on Otava's ankle. There's a wet, crunchy sound as bones shatter and tear through the surrounding muscle.")
           ...
           (case action-result
             ['hit
              (inflict-condition!
               ...
               (FreshInjury 'ankle-broken "movement 3x slower" (current-elapsed-time)))
              (when critical?
                (inflict-condition! target
                                    (current-elapsed-time)
                                    ; TODO: content
                                    (condition 'bleeding "Bleeding, should get this patched ASAP.")))
          ...
          [else
           (p
            "The Grabberkin shifts its hands onto Otava's other ankle with ease, as if it's slowly waking up, and crushes the bones in Otava's other ankle, too.")
           ...
           (case action-result
             ['hit
              (inflict-condition!
               target
               (condition 'ankle-broken (current-elapsed-time) "Can't walk, too painful."))
              (when critical?
                (inflict-condition! target
                                    (condition 'bleeding
                                               (current-elapsed-time)
             ...
             ['dead 'pc-dead]
             [else
              (error (format "resolve-anklebreaker-action!: unhandled action-result ~a"
                             action-result))])])))]
    ...
    ['grab
     ...
     (make-action
      ...
      `((define target (pc))
        (p
         "The Grabberkin seems to realize its grip is loosening. Its rotting fingers curl around Otava's ankle again with dreadful might.")
    ...
    [else (error (format "make-grabberkin-action: unknown action ~a" action-flag))]))
...
(define (get-grabberkin-action-phase-1 actor)
  (cond
    [(and (actor-has-status-of-type? (pc) 'bound)
          (> (actor-lifetime-of-status-of-type? (pc) 'bound) 1))
     (define options '(anklebreaker anklebreaker anklebreaker anklebreaker grab skip))
    ...
    [else (make-grabberkin-action actor 'grab)]))

(define (get-grabberkin-action-phase-2 actor)
  (cond
    [(and (actor-has-status-of-type? (pc) 'bound)
          (> (actor-lifetime-of-status-of-type? (pc) 'bound) 2))
     (define options '(pull-under pull-under pull-under grab skip skip))
...
(define (get-grabberkin-action actor)
  (cond
    [(and (in-combat?) (actor-in-range? actor 'close))
     (cond
       ...
       [else (make-grabberkin-action actor 'release-grip)])]
    [else '() ; not in combat, do nothing
     ]))

; implicitly, this is the pre-own-action reaction
(define (get-grabberkin-reaction actor)
  (cond
    [(not (grabberkin-hp-above-threshold? actor)) (make-grabberkin-action actor 'release-grip)]
    [else '()]))
