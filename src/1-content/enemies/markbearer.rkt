   ...
   `((define target (pc))
     (cond
       [(pc-has-sense-organ? 'eyes)
        (notice
         "The markbearer pushes his thumbs hard into Otava's eyes. There's two small pops as Otava's globes rupture.")
       ...
       [(engaged adjacent)
        (cond
          [(equal? (stance-range (actor-stance actor)) 'engaged)
           ...
           (case choice
             [(0) (make-poke-out-eyes-action actor)]
             [(1 2 3 4 5) (make-melee-action actor #:n 1 #:x 1 #:bonus 0)])]
          [else
           (define n-of-choices 6)
           (define choice (- (d 1 n-of-choices) 1))
           (case choice
             [(0 1) (make-skip-action actor)]
             [else (make-melee-action actor #:n 1 #:x 1 #:bonus 0)])])]
       [else (approach-action actor)])]
    [else (try-to-escape actor)]))
