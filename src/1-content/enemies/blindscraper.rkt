...
(define (make-blindscraper)
  (define enemy (make-actor "Blindscraper" 'blindscraper #:max-hp 3 #:size 'small))
     ...
     (cond
       [(> (actor-hp actor) 1)
        ...
        (case (actor-stance-range actor)
          [(engaged adjacent)
           (cond
             [(pc-has-sense-organ? 'eyes) ((take-random (list make-blindscrape-action)) actor)]
             [else
       ...
       [(= (actor-hp actor) 1) (try-to-escape actor)])]))
