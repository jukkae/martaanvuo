...
(define (resolve-pc-action pc-action end-round-early-with-round-status)
  (cond
    ...
    [else
     ...
     (cond
       [(initiative-based-resolution? pc-action)
       ...
       [else
        ...
        (when (equal? 'end-run pc-action-result)
          (set! round-exit-status 'end-run))
        (when (equal? 'recurse pc-action-result)
          (set! round-exit-status 'recurse))
        (when (equal? 'win-game pc-action-result)
          (set! round-exit-status 'win-game))])

     (on-end-round)
     (when (not (pc-actor-alive? (pc)))
       (set! round-exit-status 'pc-dead))
     round-exit-status]))
