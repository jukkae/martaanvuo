        ...
        (cond
          [successful?
           ...
           (define amount-string
             (if (= amount 1) (format "~a handful" amount) (format "~a handfuls" amount)))

           (info-card (tbody (tr "1d4" "=" (format "~a" amount-string))) "Forage results roll")
           (p "Otava finds crowberries and bogberries. (" (number->string amount) " handfuls.)")
           (define item (make-item 'fresh-berries #:amount amount))
           (add-item! item)])
        (if successful? 'successful 'failure))))))
