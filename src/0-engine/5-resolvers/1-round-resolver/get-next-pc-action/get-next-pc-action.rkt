  ...
  (let/ec
   ...
   (let what-do-you-do ([question-repeated? #f])
     (define (handle-meta-command meta-commands-with-keys input)
       ...
       (when (equal? meta-command-result 'restart)
         (return 'restart))
     ...
     (define fragment-decisions
       (if (null? (current-fragment-id)) '() (current-fragment-get-decisions)))
     ...
     (define location-decisions
       (if (null? fragment-decisions) (get-location-decisions (current-location)) '()))

     (define world-choices (get-world-choices (current-world) actor))

     (define choices (if (null? fragment-decisions) world-choices '()))
     ...
     (cond
       [(meta-command-valid? meta-commands-with-keys input)
       ...
       [else
        ; if whitespace only
        (notice (if (regexp-match-exact? #px"\\s*" input)
                    "Empty command"
...
(define (resolve-choice-and-produce-action! choices-with-keys input)
  ...
  (cond
    [(equal? 'resolve-clue (choice-id current-choice)) '()])

  (define action
    (cond
      [(procedure? resolution-effect) (resolution-effect)]
      ...
      [else
       (error (format "resolve-choice-and-produce-action!: unknown resolution-effect: ~a."
                      resolution-effect))]))

  ; TODO: uurgh
  (when (null? action)
    ; TODO: extract empty-action or something
    (set! action (make-action #:symbol 'empty-action #:actor (pc) #:duration 0)))
