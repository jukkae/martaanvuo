...
(require "../1-round-resolver/round-resolver.rkt"
...
(define (resolve-game game-mode #:scenario [scenario '()])
  ...
  (command-line #:once-each [("-s" "--seed")
                             seed
                             "Set random seed for new game"
  ...
  (case game-mode
    ['begin
     ...
     (if (not (null? seed-flag)) (seed-rng! seed-flag) (reset-rng!))]

    ['restart (if (not (null? seed-flag)) (seed-rng! seed-flag) (reset-rng!))]

    ['continue
     (define input-file (open-input-file "save.txt"))
     (define serialized-state (read input-file))

     (with-handlers ([exn:fail:contract:arity? (λ (exn)
                                                 (handle-broken-save)
                                                 (set! game-mode 'begin))]
                     [exn:fail:contract? (λ (exn)
                                           (handle-broken-save)
                                           (set! game-mode 'begin))])
                ...
                ['pc-dead
                 (let end-of-life-menu ([verbosity 'verbose])
                   ...
                   (hash-set! meta-commands
                              "P"
                              (cons "[P]: Proceed." (thunk (on-begin-life) (on-begin-run) 'ok)))
                   ...
                   (cond
                     [(meta-command-valid? meta-commands input)
                      (handle-meta-command meta-commands input)]
                     [else (end-of-life-menu 'abbreviated)]))]
                ['win-game (end-game 'win-game)]
                ['restart (end-game 'restart)]
                ['ok '()]
                ['recurse (on-begin-recurse-run)]
                [else (dev-note (format "Unexpected round exit status: ~a" round-exit-status))])
...
(define (handle-broken-save)
  (br)
  (prln
   "Save file is corrupt or incompatible with this revision of Martaanvuo. Delete saved progress? [D] to delete, [Q] to quit without deleting.")
