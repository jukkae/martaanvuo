...
(lazy-require ["../round-resolver/event-handler.rkt" (handle-interrupting-event!)])
    ...
    (cond
      [(route? (action-target action))
       (if (memq 'a-to-b (action-details action))
           (route-b (action-target action))
           (route-a (action-target action)))]
      [else (action-target action)]))
  (define elapsed-time 0)
  (cond
    [(not (pending? action)) (describe-begin-traverse-action from to)])
