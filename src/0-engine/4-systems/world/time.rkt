...
(require/typed "../../1-index/state.rkt" [current-elapsed-time (-> Integer)])
...
(define (time-of-day-from-iotas iotas)
  ...
  (define time-of-day
    (cond
      [(< iotas-of-current-day 100) 'morning]
...
(define (timestamp)
  ...
  (cond
    [(= 1 current-day) (format "~a ι:" (current-elapsed-time))]
    [else (format "~a/~a ι:" current-day (remainder (current-elapsed-time) day-length))]))
