...
(provide (all-from-out "conditions.rkt" "statuses.rkt"))
...
(lazy-require ["../../7-state/mutators.rkt" (get-enemies-at-range)])
; TODO: refactor
(lazy-require ["../../../1-content/enemies/grabberkin.rkt" (make-grabberkin)])
(lazy-require ["../../../1-content/enemies/blindscraper.rkt" (make-blindscraper)])
(lazy-require ["../../../1-content/enemies/markbearer.rkt" (make-markbearer)])
(lazy-require ["../../../1-content/enemies/voidfloater.rkt" (make-voidfloater)])
(lazy-require ["../../../1-content/enemies/limbtearer.rkt" (make-limbtearer)])

(define (make-actor name type #:max-hp max-hp #:size size)
...
(define (non-pc-take-damage! actor damage damage-type)
  ...
  (define result (if (= 0 (actor-hp actor)) 'dead 'hit))
  ...
  (when (equal? (actor-name actor) "Grabberkin")
...
(define (kill actor [cause-of-death '()] #:no-corpse? [no-corpse? #f])
  ...
  (notice (if (null? cause-of-death)
              (format "~a is dead." (actor-name actor))
              (format "~a is dead. Cause of death: ~a"
                      (actor-name actor)
                      (cond
                        [(symbol? cause-of-death) (describe-cause-of-death cause-of-death)]
                        [(string? cause-of-death) cause-of-death]
                        [else "NA"]))))
...
(define (make-corpse actor)
  (define corpse-name
    (Name (string-append (actor-name actor) " corpse")
          "a"
          "'s"
          (string-append (actor-name actor) " corpses")))
  (define i
    (new-item corpse-name
...
(define (inventory-contains-item-id inventory id)
  (findf (λ (i) (equal? (item-id i) id)) inventory))

(define (add-item-to-inventory! actor item)
  (cond
    [(inventory-contains-item-id (actor-inventory actor) (item-id item))
    ...
    [else (set-actor-inventory! actor (append (actor-inventory actor) (list item)))]))
...
(define (actor-has-item? actor item)
  ...
  (findf (λ (inventory-item)
           (cond
             [(symbol? inventory-item) (equal? id inventory-item)]
             [else (equal? (item-id inventory-item) id)]))
         inventory))

(define (actor-status-card actor title)
  (info-card (tbody (tr (actor-name actor) "")
                    (tr "hp:" (format "~a/~a" (actor-hp actor) (actor-max-hp actor))))
             title))

(define (inflict-status! target status-type . status-strength)
  (match status-type
    ['bound (actor-set-status! target status-type (car status-strength))]
...
(define (inflict-condition! target a-condition)
  (match (condition-type a-condition)
    ...
    ['one-eye-blind '()]
    ['both-eyes-blind '()]
    ['envenomed (actor-add-condition! target a-condition)]
  ...
  (case (condition-type condition)
    ['envenomed
     (when (= (condition-age condition) 5)
       (notice
        (format "[~a] [~a] Envenomation is now acute." (actor-name actor) (current-elapsed-time)))
       (set-Illness-state! condition 'acute))
     (when (= (condition-age condition) 100)
       (notice
        (format "[~a] [~a] Envenomation is now postacute." (actor-name actor) (current-elapsed-time)))
       (set-Illness-state! condition 'postacute))
     (when (= (condition-age condition) 700)
       (notice
        (format "[~a] [~a] Envenomation has now healed." (actor-name actor) (current-elapsed-time)))
       (actor-remove-condition-of-type! actor 'envenomed))]))

(define (get-firearm actor)
  (define items (actor-inventory actor))
  (findf (λ (item) (ranged-weapon? item)) items))

(define (make-pc-actor name
                       #:max-hp max-hp
                       #:size size
                       #:max-lp (max-lp 0)
...
(define (pc-take-damage! actor damage damage-type)
  (when (< damage 0)
    (error "pc-take-damage: damage cannot be less than 0"))

  (cond
    [(not (positive? (actor-hp actor)))
     ...
     (define result (+ death-roll (actor-hp actor)))
  ...
  (for ([i (in-range 0 number)])
    ...
    (define description
      (case i
        ...
        [(1) "left"]
        [else "right"]))
    (define enemy-stance (stance sign range description))
...
(define (set-actor-stance-range! moving-actor new-range [pc? #f])
  ...
  (when (and (not (equal? (stance-range (actor-stance moving-actor)) new-range))
             (not (and (equal? new-range 'engaged) (not (null? (get-enemies-at-range 'engaged))))))
     ...
     (format
      ...
      (format
       (case (stance-range (actor-stance moving-actor))
         ['engaged
          (if pc? "Otava is now grappling with the ~a." "The ~a is now grappling with Otava.")]
         ['adjacent (if pc? "Otava is now adjacent to the ~a." "The ~a is now adjacent to Otava.")]
         ['close (if pc? "Otava is now close to the ~a." "The ~a is now close to Otava.")]
         ['nearby (if pc? "Otava is now near ~a." "The ~a is now nearby.")]
         ['far (if pc? "Otava is now far from ~a." "The ~a is now far.")])
