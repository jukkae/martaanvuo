...
(define (actor-add-status! actor status)
  (when (not (null? actor))
    (cond
      [(not (actor-has-status-of-type? actor (status-type status)))
       (notice (format "~a: Status ~a [~a turns] added"
                       (actor-name actor)
                       (status-type status)
      ...
      [else
       ...
       (notice (format "~a: Lifetime of status ~a extended by ~a ~a, new lifetime: ~a."
                       (actor-name actor)
                       (status-type status)
                       (status-lifetime status)
...
(define (actor-has-status-of-type? actor type)
  (if (memf (λ (status) (equal? (status-type status) type)) (actor-statuses actor)) #t #f))

(define (actor-lifetime-of-status-of-type? actor type)
  (define s (findf (λ (status) (equal? (status-type status) type)) (actor-statuses actor)))
  (if s (status-lifetime s) #f))
...
(define (actor-remove-status-of-type! actor type)
  (when (not (null? actor))
    (when (actor-has-status-of-type? actor type)
      (set-actor-statuses! actor
                           (filter (λ (other) (not (equal? type (status-type other))))
