...
(lazy-require ["actor.rkt" (take-damage)])
(lazy-require ["../simulation.rkt" (advance-time-by-iotas!)])
(lazy-require ["../world/world.rkt" (get-actor)])
(lazy-require ["../../7-state/mutators.rkt" (pc)])
(lazy-require ["../../7-state/state.rkt" (current-elapsed-time)])
(lazy-require ["../../../1-content/narration/combat-narration.rkt" (display-combatant-info)])
  ...
  (case (condition-type condition)
    ...
    ['bleeding
     ...
     (cond
       ...
       [else (notice (format "Bleed check: 1d6 = 1: [~a]" bleed-damage-roll))])]
    ['trachea-fractured
     '()
...
(define (treat-injuries!)
  (for ([c (actor-conditions (pc))])
    (case (condition-type c)
      ['ankle-broken
       (p
        "Otava splints her purple, swollen ankle. She tries putting a little weight on it and immediately regrets it. There are multiple fractures in the small bones in her ankle.")
       (treat-injury! c)]
      ['both-ankles-broken
...
(define (actor-add-condition! actor condition)
  (when (not (null? actor))
    (notice (format "~a: Condition [~a] added, details: [~a]"
                    (actor-name actor)
...
(define (actor-remove-condition-of-type! actor type)
  ...
  (set-actor-conditions! actor
                         (filter (λ (other) (not (equal? type (condition-type other))))
                                 (actor-conditions actor))))

(define (actor-has-condition-of-type? actor type)
  (if (memf (λ (condition) (equal? (condition-type condition) type)) (actor-conditions actor)) #t #f))

(define (actor-get-condition-of-type actor type)
  (findf (λ (condition) (equal? (condition-type condition) type)) (actor-conditions actor)))
