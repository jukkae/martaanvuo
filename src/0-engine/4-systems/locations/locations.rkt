  ...
  (case (location-id location)
    ['magpie-hill
     (when (not (flag-set? 'magpie-hill-climbed))
       ...
       (p
        "Up on Magpie Hill there is a plateau, and on that plateau, there is a slab of old, bloodstained concrete. In the middle of the bloodstained concrete slab, there's a small bloodstained metal box, the Chamber of Astral Travel.")
       (wait-for-confirm))]
    ['lookout
     (cond
