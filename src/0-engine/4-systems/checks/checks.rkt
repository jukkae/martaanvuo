...
(define (luck-check [target-number 7] [bonus 0])
  ...
  (define result (if (>= total target-number) #t #f))
  (define result-text (if result "success" "failure"))

  (notice (format "Luck check (~a): 2d6~a = ~a – ~a" target-number bonus total result-text))
  (wait-for-confirm)
  result)
...
(define (passive-check type comparator target-number . silent)
  ...
  (define result (if check-successful? "check passed" "check failed"))
  (define sheet
    (tbody (tr text (format "~a (~a)" attribute-value (get-modifier-string modifier)) result)))

  (when (null? silent)
    (info-card sheet "Passive check"))
...
(define (attribute-check title attribute)
  ...
  (define success-string (if check-successful? ", success" ", failure"))
  (define results
    (tbody (tr "1d20" "<" "attr")
           (tr (format "~a" roll) "<" (format "~a~a" attribute success-string))))

  (info-card results (format "Attribute check: ~a" title))
...
(define (skill-check title bonus target-number)
  ...
  (define success-string (if check-successful? ", success" ", failure"))
  (define results
    (tbody (tr "2d6 + skill" ">=" "TN")
           (tr (format "~a + ~a + ~a = ~a" first-d second-d bonus roll-total)
               ">="
               (format "~a~a" target-number success-string))))

  (info-card results (format "Skill check: ~a" title))
...
(define (roll-crit? sides)
  ...
  (define crit-string (if critical? ", crit" ""))
...
(define (just-roll formula
                   #:title title
                   #:bonus [bonus '()]
                   #:on-critical-success [on-crit-success '()]
                   #:on-critical-failure [on-crit-failure '()])
  (cond
    [(string=? formula "2d6")
     (define n 2)
     (define sides 6)
    ...
    ;  (define base-sleep-time 0)
     ...
     (cond
       [(all-fulfill-predicate? results (λ (r) (= r sides)))
        (when (not (null? on-crit-success))
          (on-crit-success))]
       [(all-fulfill-predicate? results (λ (r) (= r 1)))
        (when (not (null? on-crit-failure))
          (on-crit-failure))])
     (newline)

     (notice (format "[~a]~a = [~a]"
                     (string-append* (add-between (map number->string results) " + "))
                     bonus-string
                     roll-total))
    ...
    ;  (define base-sleep-time 0)
     ...
     (newline)
     (sleep (* 2 base-sleep-time))
     (for ([dice results] [nth (in-range 0 n)])
       ...
       (display " ")
       (display "[")
       (display (number->string (if (= 10 dice) 0 dice)))
       (display "]")
       (newline))
     (define total-printed 0)
     (for ([i bonus])
     ...
     (define roll-total
       (+ (if (= (first results) 10) 0 (* 1 (first results)))
          (if (= (second results) 10) 0 (* 10 (second results)))))
     roll-total]
    [(string=? formula "1d20")
     (define n 1)
     (define sides 20)
  ...
  (cond
    [(string=? formula "2d6")
     ...
     (define check-outcome
       (cond
         [(all-fulfill-predicate? results (λ (r) (= r sides))) 'critical-success]
         [(all-fulfill-predicate? results (λ (r) (= r 1))) 'critical-failure]
    ...
    ;  (define base-sleep-time 0)
     ...
     (for ([dice results])
       (for ([i dice])
         ...
         (define sleep-time
           (cond
             [(>= total-printed tn) (* 2 base-sleep-time)]
             [(= i (- sides 2))
              (define multiplier (take-random (list 2 4 8 12)))
              (* multiplier base-sleep-time)]
             [(= i (- sides 1))
