...
(struct action
        ([symbol : Symbol] [actor-id : ActorId]
                           [duration : Natural]
...
(: make-action
   (->* (#:symbol Symbol #:actor actor #:duration Natural)
        (#:target (Maybe (U item ActorId LocationId))
         #:tags (Listof Symbol)
         #:details (Listof (U Symbol status Number))
         ...
         #:on-after-rules (Maybe Sexp))
        action))
(define (make-action #:symbol symbol
                     #:actor actor
                     #:duration duration
                     #:target [target '()]
                     #:tags [tags '()]
...
(struct standard-damage-roll ([n : Integer] [x : Integer] [bonus : Integer]) #:prefab #:mutable)
...
(define (damage-roll-formula roll)
  ...
  (format "~ad~a~a"
          ...
          (cond
            [(= 0 b) ""]
            [(negative? b) (format "~a" b)]
            [else (format "+~a" b)])))

(struct melee-attack-action
        action
        ([damage-roll : standard-damage-roll] [damage-type : (Maybe DamageType)])
  #:prefab
  #:mutable
  #:constructor-name melee-attack-action*)

(: make-melee-attack-action
   (->* (#:actor actor #:duration Natural #:target ActorId #:n Natural #:x Natural #:bonus Integer)
        (#:damage-type (Maybe DamageType)
         #:resolution-rules (Maybe Sexp)
         #:on-before-rules (Maybe Sexp)
         #:on-after-rules (Maybe Sexp))
        action))
(define (make-melee-attack-action #:actor actor
                                  #:duration duration
                                  #:target target
                                  #:resolution-rules [resolution-rules '()]
                                  #:on-before-rules [on-before-rules '()]
  ...
  (define details '())
  (define attack-roll (standard-damage-roll n x bonus))
  (melee-attack-action* symbol
                        (actor-id actor)
                        duration
                        target
                        tags
...
(define (visible-in-combat? [action : action])
  (if (or (member 'combat (action-tags action)) (member 'always (action-tags action))) #t #f))

(define (pending? [action : action])
  (if (or (equal? 'pending (action-details action))
          (if (list? (action-details action)) (member 'pending (action-details action)) #f))
      #t
      #f))

(define (free? [action : action])
  (if (member 'free (action-tags action)) #t #f))

(define (immediate-resolution? [action : action])
  (if (not (member 'delayed-resolution (action-tags action))) #t #f))

(define (initiative-based-resolution? [action : action])
  (if (member 'initiative-based-resolution (action-tags action)) #t #f))

(define (aggressive? [action : action])
  (cond
    [(equal? (action-symbol action) 'shoot) #t]
...
(define (has-tag? action tag)
  (if (memq tag (action-tags action)) #t #f))
...
(define (all-actions-of-type? [actions : (Listof action)] [type : Symbol])
  (define predicate (λ ([action : action]) (equal? (action-symbol action) type)))
