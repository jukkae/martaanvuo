...
(lazy-require ["combat.rkt" (add-combat-flag add-combat-event)])

(lazy-require ["../../1-content/narration/combat-narration.rkt"
               (get-combatant-name display-combatant-info display-pc-combatant-info)])

(lazy-require ["../4-systems/locations/locations.rkt"
               (describe-begin-traverse-action describe-finish-traverse-action
                                               describe-cancel-traverse-action
                                               location-on-enter!)])

(lazy-require ["../4-systems/world/world.rkt" (get-actor)])

(lazy-require ["../5-resolvers/1-round-resolver/event-handler.rkt" (handle-interrupting-event!)])
...
(define (resolve-melee-action! action)
  (let/ec
   ...
   (define damage-roll-result
     (+ (d (standard-damage-roll-n damage-roll) (standard-damage-roll-x damage-roll))
        (standard-damage-roll-bonus damage-roll)))
   (define body
     (tbody (tr "damage" (format "~a: [~a]" (damage-roll-formula damage-roll) damage-roll-result))))
   (info-card body title)
   (when (and target-originally-at-full-health? success? (pc-has-sense-organ? 'nose))
...
(define (resolve-successful-shoot-action! action)
  ...
  (define target (get-actor (action-target action)))
  (define title
    (format "Ranged [firearms], ~a vs ~a" (get-combatant-name actor) (get-combatant-name target)))
  ...
  (when (equal? action-result 'dead)
    (cond
      [(not (pc-actor? (action-target action)))
       (define text
         (take-random
...
(define (resolve-break-free-action! actor target [str-mod 0])
  (when (or (number? target) (symbol? target))
  ...
  (when (not statuses)
    (set! statuses '()))
  (define actor-bound-status (findf (λ (status) (equal? (status-type status) 'bound)) statuses))
  ...
  (cond
    [actor-bound-status
     ...
     (define success-string (if success? "success" "failure"))

     (notice (format "Resolution: 1d10 + bonus > TN: ~a + ~a = ~a > ~a - ~a"
                     (number->string roll)
                     (number->string bonus)
     ...
     (cond
       [success?
        (p
         "Otava pulls her ankle free and stumbles back, just far enough to be out of reach of the writhing, searching hands.")
        (award-xp! 4)
    ...
    [else ; pc not bound
     (p
      "Grabberkin hand lets loose. Otava pulls her hurt foot free and stumbles back. When Otava turns to look, rotting grabberkin has disappeared, slithered back unto the mucid dark whence it came.")
     (award-xp! 3)]))
