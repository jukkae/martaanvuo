...
(define (get-combat-choices)
  ...
  (set! combat-choices (append combat-choices (get-perceive-choices)))
  (when (actor-has-item? (pc) 'bolt-cutters)
    (set! combat-choices (append combat-choices (get-melee-choices)))
    (when (includes-enemy-of-type (append (get-enemies-at-range 'engaged)
                                          (get-enemies-at-range 'adjacent)
                                          (get-enemies-at-range 'close))
                                  'grabberkin)
      (define e
        (includes-enemy-of-type (append (get-enemies-at-range 'engaged)
                                        (get-enemies-at-range 'adjacent)
                                        (get-enemies-at-range 'close))
                                'grabberkin))
      (set! combat-choices (append combat-choices (get-harvest-choice e)))))

  (when (actor-has-item? (pc) 'revolver)
    (set! combat-choices (append combat-choices (get-ranged-choices))))
  (cond
    [(and (not (engaged?))
       ...
       (make-choice
        ...
        (λ ()
          (make-action
           ...
           #:resolution-rules
           `((define nearby-enemies
               (append (get-enemies-at-range 'nearby) (get-enemies-at-range 'close)))
             (cond
  ...
  (cond
    [(and (not (engaged?)) (not (actor-has-status-of-type? (pc) 'bound)))
     ...
     (cond
       [(and
         (empty? adjacent-enemies)
         (or (not (empty? close-enemies)) (not (empty? nearby-enemies)) (not (empty? far-enemies))))
            ...
            (make-choice 'approach
                         ...
                         (λ ()
                           (make-action #:symbol 'approach
                                        ...
                                        #:duration 1
                                        #:tags '(initiative-based-resolution fast)
                                        #:resolution-rules `((set-actor-stance-range!
                                                              (get-actor ,target-id)
                                                              ',target-stance-range
                                                              #t))))))
             ...
             (make-action
              ...
              `((for ([enemy (get-current-enemies)])
                  (case (stance-range (actor-stance enemy))
                    ['adjacent (set-actor-stance-range! enemy 'close #t)]
                    ['close (set-actor-stance-range! enemy 'nearby #t)]
                    ['nearby
                     (cond
                       ; TODO: or if pc has hearing and enemy is not silent
                       [(pc-has-sense-organ? 'eyes) (set-actor-stance-range! enemy 'far #t)]
                       [else
                        (define roll (d 2 6))
                        (notice (format "Random walk roll (2d6 >= 7): [~a]" roll))
                        (cond
                          [(>= roll 7) (set-actor-stance-range! enemy 'far #t)]
                          [else (notice "Otava stumbles in the wrong direction.")])])])))))))
        (set! combat-choices (append-element combat-choices retreat-choice))])])

  (define close-grabberkin
    (filter (λ (enemy) (equal? (actor-type enemy) 'grabberkin)) (get-enemies-at-range 'close)))

  (cond
    [(and (not (null? close-grabberkin)) (actor-has-status-of-type? (pc) 'bound))

     (define strength-mod (get-attribute-modifier-for (actor-strength (pc))))

     (define details (list 'str-mod strength-mod))

     (define target-id (actor-id (take-random close-grabberkin)))

     (define break-free-choice
       (make-choice 'pull-free
                    "Pull the leg free."
                    (λ ()
                      (make-action #:symbol 'break-free
                                   #:actor (pc)
                                   #:duration 1
                                   #:target target-id
                                   #:tags '(initiative-based-resolution fast)
...
(define (get-to-hit-bonus target)
  ...
  (cond
    [(pc-has-sense-organ? 'eyes)
     ...
     (case light-level
       ['bright
        ...
        (set! to-hit-bonus-causes-text
              (string-append to-hit-bonus-causes-text
                             (format "[eyes: ~a (bright light)]" to-hit-bonus)))]
       ['dark
    ...
    [(pc-has-sense-organ? 'sonar)
     ...
     (when (> bonus 0)
       (set! to-hit-bonus-causes-text
             (string-append to-hit-bonus-causes-text (format "[sonar: ~a]" to-hit-bonus))))])
...
(define (make-named-attack action-name target)
  ...
  (case action-name
    ['strike
     ...
     (define bonus-text
       (cond
         [(not (negative? to-hit-bonus)) (format "+~a" to-hit-bonus)]
         [else (format "~a" to-hit-bonus)]))
      ...
      (λ ()
        ...
        (make-action
         ...
         `((define actor (pc))
           ...
           (define title
             (format "Melee attack, ~a vs ~a" (get-combatant-name actor) (get-combatant-name target)))
           (define target-number 10)
           (define to-hit-roll-result (+ (d 2 6) ,to-hit-bonus))
           (define success? (>= to-hit-roll-result target-number))
           (define success-text (if success? "success" "fail"))
           ...
           (define body
             (prune
              (tbody (tr "to hit"
                     ...
                     (when success?
                       (tr "damage"
                           (format "~ad~a ~a: [~a]" 1 2 ,damage-bonus-text damage-roll-result))))))
           (info-card body title)
           (define action-result 'ok) ; TODO: likely not useful anymore
           (when success?
             (set! action-result (take-damage target damage-roll-result 'melee)))
      ...
      (λ ()
        ...
        (make-action
         ...
         `((define actor (pc))
           (define target (get-actor ,target-id))
           (define title
             (format "Strangle, ~a vs ~a" (get-combatant-name actor) (get-combatant-name target)))
           ...
           (define action-result 'ok) ; TODO: likely not useful anymore
           (cond
             [(not (actor-has-condition-of-type? target 'trachea-fractured))
              ...
              (case attack-roll
                [(1)
                 (notice (format "Otava can't get a good grip on the ~a's neck."
                                 (get-combatant-name target)))]
                [(2 3 4 5)
                 (notice (format "There's a crack as the ~a's windpipe breaks."
                                 (get-combatant-name target)))
                 (inflict-condition!
                  target
                  (condition 'trachea-fractured (current-elapsed-time) "Broken windpipe."))
    ...
    ['smash-their-head-in
     (make-choice
      'attack
      (format "Smash in the head of the ~a." (get-combatant-name target))
      (λ ()
        (define target-id (actor-id target))
        (make-action
         #:actor (pc)
         ...
         `((define actor (pc))
           (define target (get-actor ,target-id))
           (define title
             (format "smash, ~a vs ~a" (get-combatant-name actor) (get-combatant-name target)))
           #;(when (not (equal? (stance-range (actor-stance target)) 'engaged))
               (notice (format "Otava has to engage the ~a" (get-combatant-name target)))
               (set-actor-stance-range! target 'engaged #t))
           (define action-result 'ok) ; TODO: likely not useful anymore
           ...
           (case attack-roll
             [(1)
              (notice (format "Otava can't get a grip on the ~a's head."
                              (get-combatant-name target)))]
             [(2 3)
              (notice (format "Otava slams the ~a's head a couple of times against the hard rock."
                              (get-combatant-name target)))
              (set! action-result (take-damage target 1 'bashing))]
             [(4 5)
              (notice (format "Otava smashes the ~a's head hard against the heavy rock."
                              (get-combatant-name target)))
              (set! action-result (take-damage target 2 'bashing))]
             [(6)
              (p
               (format
                "Otava bashes the ~a's head against a fucking sharp rock with all of her might. There's a crunch and the side of the head caves in, mangling the face of the ~a."
                (get-combatant-name target)))
      ...
      (λ ()
        ...
        (make-action
         ...
         `((define actor (pc))
           ...
           (case attack-roll
             [(1)
              (notice (format "Otava throws the ~a, and herself, off the cliff."
                              (get-combatant-name target)))
             ...
             [(2)
              (notice
               (format
                "Otava almost throws herself off the cliff by accident before regaining her balance."))
    ...
    [else
     (make-choice 'attack
                  ...
                  (λ ()
                    (define target-id (actor-id target))
                    (make-melee-attack-action #:actor (pc)
                                              #:duration 1
                                              #:target target-id
                                              #:n 1
                                              #:x 2
...
(define (get-melee-choices)
  ...
  (for ([i (in-range 0 (length targets))])
    ...
    (cond
      [(or (equal? (stance-range stance) 'engaged) (equal? (stance-range stance) 'adjacent))
       (for ([action-name (list 'strangle)])
         (define choice (make-named-attack action-name target))
         (set! combat-choices (append-element combat-choices choice)))])
    (cond
      [(or (equal? (stance-range stance) 'engaged))
...
(define (get-harvest-choice target)
  (list (make-choice
         ...
         (λ ()
           (define target-id (actor-id target))
           (make-action #:symbol 'harvest-finger
                        #:actor (pc)
                        #:duration 1
                        #:target target-id
                        #:tags '(initiative-based-resolution fast)
                        #:resolution-rules `((add-item! 'grabberkin-finger) (wait-for-confirm)))))))
...
(define (get-ranged-choices)
  ...
  (define all-choices
    (for/list ([i (in-range 0 (length targets))])
      ...
      (list (when (and (not (flag-set? 'aware-of-being-out-of-ammo))
