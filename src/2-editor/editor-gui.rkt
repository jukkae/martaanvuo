...
(define (label-bitmap-proc l)
  (let ([label (first l)] [image? (second l)] [file (third l)])
    (or (and image?
             (or (and file
                      (let ([bmp (make-object bitmap% file 'unknown/mask)]) (and (send bmp ok?) bmp)))
                 "<Bad Image>"))
        label)))
...
(define (list->font l)
  (with-handlers
      ([exn:fail?
        (λ (e) (send/apply the-font-list find-or-create-font (cons (first l) (rest (rest l)))))])
...
(define tab-world #f)
(define (editor-gui-init #:main-window-width (main-window-width 1200)
                         #:main-window-height (main-window-height 800)
                         #:fragment-file-selector-callback
                         (fragment-file-selector-callback (lambda (list-box control-event) (void)))
  ...
  (set! tab-panel-20641
        (new (class tab-panel%
               ...
               (define/public (active-child n)
                 (send this change-children (lambda (children) (list (list-ref child-panels n))))))
             (parent main-window)
             (choices (list))
  ...
  (set! fragment-file-selector
        (new list-box%
             ...
             (style ((lambda (l) (list* (first l) (second l) (third l)))
                     (list 'single 'vertical-label '())))
             (font (list->font (list 12 "Helvetica" 'default 'normal 'normal #f 'default #f)))
             (selection 0)
             (enabled #t)
             (vert-margin 2)
             (horiz-margin 2)
